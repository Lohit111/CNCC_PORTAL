...
class MainTypeController:
    ...
    def update(db: Session, main_type_id: int, data: dict):
        ...
        try:
            ...
            if not updated:
                raise HTTPException(
                    status_code=500, detail="Main type update failed")
            ...
            logger.error(
                f"Failed to update main type {main_type_id}: {str(e)}")
            raise HTTPException(
                status_code=500, detail=f"Database error: {str(e)}")
    ...
    def delete(db: Session, main_type_id: int):
        ...
        try:
            ...
            logger.error(
                f"Failed to delete main type {main_type_id}: {str(e)}")
            raise HTTPException(
                status_code=500, detail=f"Database error: {str(e)}")
...
class SubTypeController:
    ...
    def get_by_main_type(db: Session, main_type_id: int):
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(sub_types)} sub types for main type {main_type_id}")
            return sub_types
    ...
    def update(db: Session, sub_type_id: int, data: dict):
        ...
        try:
            ...
            if not updated:
                raise HTTPException(
                    status_code=500, detail="Sub type update failed")
