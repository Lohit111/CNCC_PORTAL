...
class AdminArchivePage extends ConsumerWidget {
  const AdminArchivePage({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final cs = Theme.of(context).colorScheme;
    final state = ref.watch(adminProvider('archive'));
    return state.when(
      ...
      data: (data) {
        ...
        return RefreshIndicator(
          onRefresh: () async =>
              ref.read(adminProvider('archive').notifier).refresh(),
          child: ListView.builder(
            padding: const EdgeInsets.fromLTRB(16, 12, 16, 80),
            itemCount: data.requests.length,
            itemBuilder: (_, i) => AdminRequestActionCard(
                detail: data.requests[i], category: 'archive'),
          ),
        );
      },
    );
